...
if [ -n "${KLEAF_INTERNAL_PERFERRED_KERNEL_DIR}" ]; then
  if [ "${KERNEL_DIR}" != "${KLEAF_INTERNAL_PERFERRED_KERNEL_DIR}" ]; then
    # If kernel_build.makefile is set and inconsistent with the value in build config, print a error.
    echo "ERROR: You specified kernel_build.makefile to be below ${KLEAF_INTERNAL_PERFERRED_KERNEL_DIR}," >&2
