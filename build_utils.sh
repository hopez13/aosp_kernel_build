...
function create_modules_staging() {
  ...
  # set. So back it up first so we can restore it later once the modules.load
  # file is generated.
  ...
  if [ -z "${TRIM_UNUSED_MODULES}" ]; then
    ...
    # modules. This allows us to keep an account of all the modules that are
    # contained within the staging directory.
