...
- `error_message`: If set, and the value of the flag is the same as the default value,
  emit a build error.
- `warn_message`: If set, and the value of the flag is the same as the default value,
  emit a warning.
"""
...
def _string_flag_impl(ctx):
    ...
    if ctx.attr.error_message and ctx.attr.build_setting_default == ctx.build_setting_value:
        fail("{} has unsupported value {}. {}".format(
            ctx.label,
            ctx.build_setting_value,
            ctx.attr.error_message,
        ))
    if ctx.attr.warn_message and ctx.attr.build_setting_default == ctx.build_setting_value:
        # buildifier: disable=print
        print("\nWARNING: {} has deprecated value {}. {}".format(
            ctx.label,
            ctx.build_setting_value,
