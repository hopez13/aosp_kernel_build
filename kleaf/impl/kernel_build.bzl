...
def _get_dot_config_impl(subrule_ctx, config_out_dir):
    """Gets .config from kernel_config's out_dir."""
    hermetic_tools = hermetic_toolchain.get(subrule_ctx)
...
def _create_infos(
    ...
    if ctx.attr.keep_dot_config:
        default_info_files.append(_get_dot_config(ctx.file.config))
