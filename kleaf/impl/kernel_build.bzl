           ...
           if grep -q -E -e "\\bmodules\\b|[.]ko\\b" <<< "{make_goals}" ; then
               if grep -q -E -e "[.]ko\\b" <<< "{make_goals}" ; then
                   ...
                   for module in {make_goals}; do
                       # For single_module builds, we don't really care about
                       # the module ordering here. With fw_devlink enabled in
                       # GKI, the probe ordering should be handled based on
                       # device dependencies.
