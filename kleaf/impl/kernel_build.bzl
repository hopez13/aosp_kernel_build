...
def _gcno_common_impl(ctx, file_mappings, rsync_cmd, mappings, extra_inputs, gcno_dir):
    ...
    if ctx.attr._gcov[BuildSettingInfo].value:
        ...
        grab_gcno_cmd = """
            ...
            {rsync_cmd}
            # Merge mappings.
            {print_gcno_mapping} {file_mappings} {mappings} > {gcno_mapping}
...
def get_grab_gcno_step(ctx, src_dir, is_kernel_build):
    ...
    if ctx.attr._gcov[BuildSettingInfo].value:
        ...
        if base_kernel and base_kernel[GcovInfo].gcno_mapping:
            file_mappings = "--file_mappings {}".format(base_kernel[GcovInfo].gcno_mapping.path)
        ...
        mappings = "--mappings {src_dir}:{gcno_dir}".format(src_dir = src_dir, gcno_dir = gcno_dir.path)
