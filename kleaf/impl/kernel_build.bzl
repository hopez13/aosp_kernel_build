...
def _gcno_common_impl(ctx, file_mappings_cmd, rsync_cmd, mappings_cmd, extra_inputs, gcno_dir):
    """Returns a step for handling `*.gcno`files and mappings.

    Args:
        ctx: Context from the rule.
        file_mappings_cmd: Additional file mappings, separated by spaces.
        rsync_cmd: All directories rsync commands.
        mappings_cmd: SRC:DEST mappings, separated by spaces.
        extra_inputs: inputs.
    ...
    if ctx.attr._gcov[BuildSettingInfo].value:
        ...
        grab_gcno_cmd = """
            ...
            {rsync_cmd}
            # Merge mappings.
            {print_gcno_mapping} {file_mappings_cmd} {mappings_cmd} > {gcno_mapping}
        ...
        """.format(
            ...
            file_mappings_cmd = file_mappings_cmd,
            mappings_cmd = mappings_cmd,
...
def get_grab_gcno_step(ctx, src_dir, is_kernel_build):
    ...
    if ctx.attr._gcov[BuildSettingInfo].value:
        ...
        if base_kernel and base_kernel[GcovInfo].gcno_mapping:
            file_mappings_cmd = "--file_mappings {}".format(base_kernel[GcovInfo].gcno_mapping.path)
        ...
        mappings_cmd = "--mappings {src_dir}:{gcno_dir}".format(src_dir = src_dir, gcno_dir = gcno_dir.path)
