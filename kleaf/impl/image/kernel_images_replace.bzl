...
def _kernel_images_replace_impl(ctx):
    ...
    args.extend([
        ...
        "boot_images(",
        """\
# FIXME: If vendor_boot_name is not set, this target should be deleted.
vendor_boot_image(
  # FIXME: unpack_ramdisk = ?","""
    ])
    args.extend([
        "--replace",
        "build_boot = True",
    ...
    content = hermetic_tools.run_setup + """#!/bin/sh -e
        query_output=$(mktemp)
        # https://github.com/bazelbuild/bazel/issues/6341: Use gencquery instead
        ( cd $BUILD_WORKSPACE_DIRECTORY && tools/bazel cquery --output=build {targets} "$@" > "${{query_output}}")
        {bin} "${{query_output}}" {process_args}
        rm -f "${{query_output}}"
    """.format(
        bin = ctx.executable._bin.short_path,
        targets = shell.quote(" union ".join([str(target.label) for target in ctx.attr.targets])),
