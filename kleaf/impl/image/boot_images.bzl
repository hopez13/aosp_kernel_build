...
def _build_boot_or_vendor_boot(
    ...
    if vendor_ramdisk_binaries:
        ...
        """.format(
            written = written_vendor_ramdisk_binaries.depset_file,
        )
    ...
    command += """
             # Build boot images
               ...
               {search_and_cp_output} --srcdir ${{DIST_DIR}} --dstdir {outdir} {out_files}
    ...
    """.format(
        ...
        outdir = outdir,
        out_files = " ".join([out.path for out in out_files]),
