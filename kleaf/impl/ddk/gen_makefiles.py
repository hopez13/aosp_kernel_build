...
def _get_ddk_marker(
    ...
    suffix = "ddk_marker" + suffix + ".c"
    ddk_marker = output_dir / suffix
...
def _handle_ddk_marker(
    ...
    if any([src.name == "ddk_marker.c" for src in rel_srcs_flat]):
        ddk_marker_suffix = "_internal"
    if kernel_module_out.with_suffix(".c") == "ddk_marker.c":
        ddk_marker_suffix = "_internal"
    ddk_marker = _get_ddk_marker(out_cflags_path.parent, ddk_marker_suffix)
    # Depending on the number of files, choose an appropriate path for tagging.
    if len(rel_srcs_flat) > 1:
        rel_srcs.append({"files": [pathlib.Path(ddk_marker.name)]})
    else:
