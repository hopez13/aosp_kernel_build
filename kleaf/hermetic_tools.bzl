...
def _platform_transition_impl(_settings, _attr):
    return {"//command_line_option:platforms": "//build/kernel/kleaf/impl:prebuilts_exec_platform"}
...
hermetic_tools = rule(
    ...
    attrs = {
        ...
        "symlinks": attr.label_keyed_string_dict(
            allow_files = True,
            doc = """
                A dictionary, where keys are labels to an executable, and
        ...
        "extra_args": attr.string_list_dict(
            doc = """
                Keys are names to the tool (see `symlinks`). Values are
