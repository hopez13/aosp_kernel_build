...
Set `strip_prefix = strip_prefix.files_only()` so the directory structure within
`destdir` is flattened.

Set `destdir` so there's less typing at build time. For example:
