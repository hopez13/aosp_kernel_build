...
| <a id="kernel_build-check_defconfig"></a>check_defconfig |  If `True`, runs `check_defconfig` after `make defconfig`.<br><br>This can only be set to true if `defconfig` is set, and `pre_defconfig_fragments` and `post_defconfig_fragments` is not set.<br><br>If `True`, after `make defconfig`, also runs `make savedefconfig` and checks that the result is exactly the same as `defconfig`.   |  `None` |
